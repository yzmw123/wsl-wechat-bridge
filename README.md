...
然后一键同步到 WSL/Linux 微信里。小组件分成“剪贴板”和“运行状态”两页：剪贴板页负责预览/编辑内容并同步到 WSL，运行状态页负责看统一监听、最近操作输出和手动启停监听。你也可以把它当作日常主操作入口：在这里启动/关闭 WSL 微信，读取 Windows 剪切板，把内容同步进 Linux 微信，也可以手动把 WSL/Linux 文本剪切板拉回 Windows。

![WSL 剪切板同步小组件](docs/images/clipboard-widget.png)

小组件适合处理 Windows -> Linux 微信的主动同步：复制文字、图片或文件后，点一下“同步到 WSL”，再切到 Linux 微信里粘贴。需要把 Linux/WSL 里的文本拉回 Windows 时，就切到“运行状态”页点“读取WSL剪切板”，在自动监听偶发延迟时手动补一次。
...
Windows 文本 -> Linux 微信默认走小组件里的“同步到 WSL”按钮，或者手动运行 `winclip2wechat`。Linux/WSL 文本 -> Windows 可以走统一监听，也可以在小组件“运行状态”页点“读取WSL剪切板”，等价于手动运行 `wechatclip2win`。这样做是为了避免两个系统互相监听文本后出现循环同步、抢剪切板、越用越乱。
