...
然后一键同步到 WSL/Linux 微信里。你也可以把小组件当作日常主操作入口：在这里启动/关闭 WSL 微信，查看剪切板监听状态，读取 Windows 剪切板，并把内容同步进 Linux 微信。

![WSL 剪切板同步小组件](docs/images/clipboard-widget.png)

小组件适合处理 Windows -> Linux 微信的主动同步：复制文字、图片或文件后，点一下“同步到 WSL”，再切到 Linux 微信里粘贴。这个“同步到 WSL”按钮是关键动作，尤其是 Windows 文本要进 Linux 微信时。
