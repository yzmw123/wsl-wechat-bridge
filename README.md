...
新装的 WSL/Ubuntu 通常没有可直接给 Linux GUI 应用使用的中文输入法。项目会设置 fcitx 环境变量并启动 fcitx4；默认可使用 `fcitx-pinyin`，安装 `sogoupinyin` 后也可切换到搜狗拼音 4.x。
...
微信仍在运行但输入法异常时，可以点击“WSL 剪切板同步”小组件首页的 `重置输入法`，或运行 `wsl -d Ubuntu-22.04 -- wechat-input-reset`。为避免搜狗插件继续持有已删除队列的旧句柄，这个动作会受控重启当前嵌套微信桌面与 fcitx/搜狗会话、清理当前用户和显示号的残留队列，并在进入第一个 WSL 输入框后自动启用搜狗；未发送的输入内容请先保存。
...
然后一键同步到 WSL/Linux 微信里。小组件分成“剪贴板”和“运行状态”两页：剪贴板页负责预览/编辑内容并同步到 WSL，运行状态页负责看统一监听、最近操作输出和手动启停监听；“运行状态”页签上的小圆点会直接显示监听状态，黄代表未监听，绿代表正常。你也可以把它当作日常主操作入口：在这里启动/关闭 WSL 微信、点击 `重置输入法` 清理残留并切回搜狗、读取 Windows 剪切板、把内容同步进 Linux 微信，也可以手动把 WSL/Linux 文本剪切板拉回 Windows。
