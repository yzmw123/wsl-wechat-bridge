...
新装的 WSL/Ubuntu 通常没有可直接给 Linux GUI 应用使用的中文输入法。项目会设置 fcitx 环境变量并启动 fcitx4；默认可使用 `fcitx-pinyin`，另外支持用户自行安装的 `sogoupinyin` 4.x。仓库和 `-InstallDependencies` 不包含、也不会下载安装搜狗 `.deb`。

搜狗在 WSL 中还依赖 POSIX 消息队列。项目启动自己管理的 fcitx 前会确保 `/dev/mqueue` 已挂载，并只清理当前用户、当前嵌套显示号的旧搜狗队列；正常关闭时也会回收对应进程和队列，避免出现“英文正常、切到搜狗后输入严重卡顿”。诊断信息写入 `~/.cache/wechat-desktop/fcitx5.log`（沿用旧日志文件名），`wechat-desktop-status` 会显示 fcitx/搜狗进程、挂载状态和队列数量。如果日志提示无法自动挂载，先执行 `sudo mkdir -p /dev/mqueue && sudo mount -t mqueue none /dev/mqueue`。

微信仍在运行但搜狗异常时，可以点击“WSL 剪切板同步”小组件首页的 `重置搜狗输入法`，或运行 `wsl -d Ubuntu-22.04 -- wechat-input-reset`。小组件会先调用 `wechat-input-reset --check`；没有安装 fcitx4/搜狗 4.x 时按钮会禁用，`fcitx-pinyin` 仍可正常使用，但不冒充搜狗重置。为避免搜狗插件继续持有已删除队列的旧句柄，重置会在确认后受控重启当前嵌套微信桌面与 fcitx/搜狗会话、清理当前用户和显示号的残留队列，并在进入第一个 WSL 输入框后自动启用搜狗。

输入法重置功能的兼容范围是：Windows 11 + WSL2、Ubuntu/Debian 风格的 `sogoupinyin` 包和 `/opt/sogoupinyin/files/bin` 布局、本项目启动的 Xephyr 嵌套微信桌面、fcitx4。它不支持 fcitx5、IBus、其他输入法、原生 Linux 桌面或绕过本项目直接启动的 WSLg 微信。停止和重置按受管 PID 与 `DISPLAY` 限定；同一 WSL 用户仍建议只运行一个本项目托管桌面。
...
然后一键同步到 WSL/Linux 微信里。小组件分成“剪贴板”和“运行状态”两页：剪贴板页负责预览/编辑内容并同步到 WSL，运行状态页负责看统一监听、最近操作输出和手动启停监听；“运行状态”页签上的小圆点会直接显示监听状态，黄代表未监听，绿代表正常。你也可以把它当作日常主操作入口：在这里启动/关闭 WSL 微信、点击 `重置搜狗输入法` 清理残留并切回搜狗、读取 Windows 剪切板、把内容同步进 Linux 微信，也可以手动把 WSL/Linux 文本剪切板拉回 Windows。
...
默认通知路径优先使用 D-Bus 通知、X11 attention、窗口标题里的未读信号，以及会话列表中的数字未读角标。数字角标 watcher 只把带浅色数字笔画的红色角标当作未读消息；公众号、服务号和免打扰会话常见的纯红点不会触发提醒。消息/session 文件活动只用于诊断日志，不再直接触发任务栏闪动。
...
- 修复搜狗拼音在 WSL 嵌套桌面中因消息队列残留导致的严重输入卡顿，并补充运行状态诊断。
- “WSL 剪切板同步”小组件新增 `重置搜狗输入法` 按钮，可安全清理当前项目桌面的残留队列并重新启动微信、fcitx4 和搜狗拼音。
- 重置功能增加环境检测、并发锁、确认提示和进程范围保护；不符合 `WSL2 + Xephyr + fcitx4 + 搜狗拼音 4.x` 条件时会禁用按钮，避免误操作其他输入法或桌面会话。
- 安装器会保存选定的 WSL 发行版，Windows 启动器、诊断脚本和帮助文档均已同步适配。
- 完整的逐项维护记录和验证结果见 [`docs/CHANGELOG.md`](docs/CHANGELOG.md)。
...
- WSL 里具备必要依赖，例如 `x11-utils`、`x11-apps`、`xclip`、`wmctrl`、`xdotool`、`xserver-xephyr`、`openbox`、`tint2`、`dbus-x11`、`fcitx`、`fcitx-pinyin`、`python3`、`python3-dbus`、`python3-gi`；使用搜狗拼音时还需要其 Qt Quick/QML、gsettings-qt、OpenMP 和 Xss 运行库，安装脚本的 `-InstallDependencies` 会补齐这些库。
...
安装器会把本次 `-Distro` 值写入 Windows 辅助目录的 `distro.txt`，因此桌面快捷方式、小组件、监听器和应用控制不会在非默认发行版上错误回退到 `Ubuntu-22.04`。同时会把 WSL 侧命令安装到：
...
# 默认 1；数字未读角标 watcher，设为 0 可关闭定期截图分析
...
先只读检查当前发行版是否具备 fcitx4 + 搜狗拼音 4.x 重置能力：

```powershell
wsl -d Ubuntu-22.04 -- wechat-input-reset --check
```

重启当前嵌套微信桌面、重置搜狗输入法并切回搜狗（命令行调用前请先保存未发送内容）：
