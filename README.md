...
默认通知路径优先使用 D-Bus 通知、X11 attention、窗口标题里的未读信号等低成本信号；如果 Linux 微信只产生消息/session 文件活动，文件活动 watcher 会作为后备只触发任务栏闪动，不受“消息弹窗”开关关闭影响。实验性的未读角标截图 watcher 默认关闭，需要在配置里显式开启。
